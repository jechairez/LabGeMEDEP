...
network<-function(t, state, parameters) {
	with(as.list(c(state, parameters)),{
		...
		dMYB77 <- ((-exp(0.5*h)+exp(-h*(w_MYB77)))/((1-exp(0.5*h))*(1+exp(-h*(w_MYB77-0.5)))))-(lambda*MYB77)
...
times <-seq(0,20,by=0.01)
	...
	salida.pdf <- paste("gamas",".pdf",sep="")
	pdf(salida.pdf, height=4)
	ejeY <- 0
	for(m in 1:length(state)){ ejeY <- max(ejeY,o[,(m+1)])} #Solo para sacar el valor maximo para el eje Y
	plot(o[,1],o[,2], ylim=c(0,ejeY), type = "l", col=colores[1], xlab="Timesteps", ylab="Relative concentrations")
        for(n2 in 2:length(state)){ lines(o[,1],o[,(n2+1)], col=colores[n2])}
        legend("topright", lwd=2, names(state), col=colores, cex=0.7, box.lwd = 0, box.col = "white", bg = "white") #leyenda
dev.off()
